...
set BUILD_DIR_FOUND=false
set BUILD_FILES_FOUND=true
for %%f in ("Source/pdex.dll" "Source/pdex.elf") do (
	echo "%%~f"
	if not exist "%%~f" (set BUILD_FILES_FOUND=false)
)
for /d %%d in ("*.pdx") do (
	set BUILD_DIR_FOUND=true
	echo "%%~d"
	if not exist "%%~d/pdex.bin" (set BUILD_FILES_FOUND=false)
	if not exist "%%~d/pdex.dll" (set BUILD_FILES_FOUND=false)
)
if "%BUILD_DIR_FOUND%"=="false" (echo -- BUILD FAILURE && exit /b 1)
if "%BUILD_FILES_FOUND%"=="false" (echo -- BUILD FAILURE && exit /b 1)
